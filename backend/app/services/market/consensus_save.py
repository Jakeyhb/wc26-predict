...
def get_latest_consensus(
    ...
    """Retrieve the most recent market consensus for a given matchup.

    Returns a dict with consensus probs + metadata, or None.
    """
    if not DB_PATH.exists():
        return None
    ...
    try:
        ...
        if row:
            return {
                ...
                "consensus_home": row["consensus_home"],
                "consensus_draw": row["consensus_draw"],
                "consensus_away": row["consensus_away"],
                "bookmaker_count": row["bookmaker_count"],
                "provider_count": row["provider_count"],
                "overround_avg": row["overround_avg"],
                "confidence": row["confidence"],
                "fetch_status": row["fetch_status"] if "fetch_status" in row.keys() else "",
                "provider_names": row["provider_names"] if "provider_names" in row.keys() else "",
            }
        return None

    except Exception:
        logger.debug("Failed to read market consensus", exc_info=True)
        return None
